...
BLOG_AI_URL=$(ssm_get_or "$BLOG_SSM/BLOG_AI_BASE_URL" "")
BLOG_AI_UPSTREAM="${BLOG_AI_URL#http://}"
BLOG_AI_UPSTREAM="${BLOG_AI_UPSTREAM#https://}"

write_env "$COMPOSE_ENV" <<EOF
BLOG_API_IMAGE=$NEW_IMAGE
CLOUDFRONT_SECRET=$CF_SECRET
BLOG_AI_UPSTREAM=$BLOG_AI_UPSTREAM
