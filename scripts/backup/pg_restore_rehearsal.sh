...
docker exec blog-postgres psql -U postgres -d postgres -v ON_ERROR_STOP=1 \
  -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '$TEST_DB';" \
  -c "DROP DATABASE IF EXISTS $TEST_DB;" \
  -c "CREATE DATABASE $TEST_DB;"

gunzip -c "$LATEST_FILE" | docker exec -i blog-postgres psql -U postgres -d "$TEST_DB" -v ON_ERROR_STOP=1 >/dev/null

TABLE_COUNT=$(docker exec blog-postgres psql -U postgres -d "$TEST_DB" -Atc "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public';")
if [ "${TABLE_COUNT:-0}" -le 0 ]; then
  echo "Restore rehearsal failed: restored DB has no public tables"
  exit 1
fi

docker exec blog-postgres psql -U postgres -d postgres -v ON_ERROR_STOP=1 -c "DROP DATABASE IF EXISTS $TEST_DB;" >/dev/null
