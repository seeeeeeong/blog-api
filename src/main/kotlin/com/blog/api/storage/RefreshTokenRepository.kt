...
import com.blog.api.core.domain.RefreshToken
...
class RefreshTokenRepository(
    ...
    fun save(tokenInfo: RefreshToken) {
        try {
    ...
    fun findByTokenId(tokenId: String): RefreshToken? {
        return try {
            ...
            objectMapper.readValue<RefreshToken>(value)
        } catch (e: Exception) {
    ...
    fun markAsUsed(tokenId: String) {
        try {
            ...
            val updatedInfo = tokenInfo.copy(isUsed = true)
            save(updatedInfo)
    ...
    fun deleteByFamilyId(familyId: String) {
        try {
            ...
            userId?.let { resolvedUserId ->
                redisTemplate.opsForSet().remove(getUserFamiliesKey(resolvedUserId), familyId)
            }
