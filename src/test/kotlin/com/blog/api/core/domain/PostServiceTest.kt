...
class PostServiceTest {
    ...
    fun `published 글 조회 시 조회수를 증가시킨다`() {
        ...
        val postViewService = mock(PostViewService::class.java)
        val service = fixture(postRepository, postViewService)
        ...
        verify(postViewService).incrementIfNeeded(1L, "127.0.0.1")
    }
    ...
    fun `draft 글은 작성자 본인 admin 이면 조회 가능하다`() {
        ...
        verify(postViewService, never()).incrementIfNeeded(2L, "127.0.0.1")
    }
    ...
    fun `draft 글은 비공개로 유지된다`() {
        ...
        verify(postViewService, never()).incrementIfNeeded(3L, "127.0.0.1")
    }
    ...
    private fun fixture(
        ...
        return PostService(
            postRepository = postRepository,
            postHtmlCacheService = mock(PostHtmlCacheService::class.java),
            postViewService = postViewService,
            postRankingService = mock(PostRankingService::class.java),
